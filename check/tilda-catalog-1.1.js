...
function t_store_init(recid, opts) {
    ...
    var loadMoreBtn = t_store_get_loadMoreBtn_html(opts);

    if (opts.sidebar) {
        $('.t951__cont-w-filter').append(loadMoreBtn);
    } else {
    ...
    */
    ...
    if (window.location.href.indexOf('s_recid=') !== -1) {
        ...
        if (recIdFromURL == recid) {
            ...
            var optsBasedOnURL = t_store_updateOptionsBasedOnUrl(opts);
            if(optsBasedOnURL) {
                opts = optsBasedOnURL;
    ...
    try {
        if (opts.verticalAlignButtons) {
            ...
            $(window).bind('displayChanged', function () {
                t_store_verticalAlignButtons(recid, opts);
            });
        }
    } catch (e) {
...
function t_store_initStoreParts(recid, opts) {
    ...
    rec.find('.js-store-parts-switcher').on('click', function () {
        ...
        } else if (storepartFromSearchParams !== storeparttitle) {
...
function t_store_process(productsArr, recid, opts, isNextSlice, isRelevantsShow, obj) {
    ...
    if (isRelevantsShow && opts.relevants_slider && (productsArr.length > blocksInRowForRelevant || $(window).width() <= 960)) {
        ...
        productsHtml +=
            ...
            blocksInRowForRelevant +
            '" data-slider-transition="' +
            animationspeed +
            '" data-slider-with-cycle="true" data-slider-cycle="yes" data-slider-correct-height="false" data-auto-correct-mobile-width="false">';
    ...
    $.each(productsArr, function (i, product) {
        if (!isRelevantsShow || (isRelevantsShow && !opts.relevants_slider)) {
    ...
    if ($('.t706__cartwin').length > 0) {
        ...
        console.log('Error: cart block is not added to this page');
    }
...
function t_store_get_productPopup_html(recid, opts) {
    ...
    str += '                            ' + t_store_get_productPopup_portionUnit_html(opts);
...
function t_store_get_productPopup_portionUnit_html(opts) {
    var str = '';
    var styleAttr = '';
    var styleStr = 'color:' + opts.price.color;
    styleStr += opts.price.fontWeight ? 'font-weight:' + opts.price.fontWeight + ';' : '';
    styleAttr = styleStr !== '' ? 'style = "' + styleStr + '"' : '';

    str += '<div class="js-product-portion-unit t-name t-name_md" ' + styleAttr + '>';
    str += '    ' + '<div class="js-product-portion"></div>';
    str += '    ' + '<div class="js-product-unit"></div>';
    str += '</div>';
...
function t_store_get_productPopup_closeIcon_html(opts) {
    var str = '';
    var fillColor = opts.popup_opts.iconColor ? opts.popup_opts.iconColor : '#fff';
    str += '<div class="t-popup__close">';
    ...
    str += '            <g stroke="none" stroke-width="1" fill="' + fillColor + '" fill-rule="evenodd">';
    str +=
...
function t_store_get_loadMoreBtn_html(opts) {
    var str = '';
    var btnSizeClass = opts.btnSize === 'sm' ? 't-btn_xs' : 't-btn_sm';
    str += '<div class="t-store__load-more-btn-wrap t-align_center">';
...
function t_store_get_handIcon_html() {
    var str = '';
    str += '<div class="t-store__scroll-icon-wrapper">';
    str += '      <img style="height: 42px" src="https://static.tildacdn.com/tild3536-3732-4061-a132-656134303361/hand.svg">';
    str += '</div>';
    return str;
}
...
function t_store_get_productCard_getWrapperStylesStr(opts) {
    ...
    if (opts.prodCard.shadowSize || opts.shadowOpacity) {
        var size = opts.prodCard.shadowSize ? parseInt(opts.prodCard.shadowSize, 10) + 'px' : '20px';
...
function t_store_get_productCard_img_html(product, opts) {
    ...
    var style = opts.imageHeight && !opts.isFlexCols ? 'style="padding-bottom:' + (parseInt(opts.imageHeight, 10) * 100) / opts.colWidth + '%;"' : '';
    ...
    if (imgSrc !== '') {
        ...
        str += '<div class="' + wrapperClassStr + '" ' + style + '>';
...
function t_store_get_productCard_txtAndPrice_html(product, edition, opts, strImg) {
    ...
    str += '            <div class="js-store-price-wrapper t-store__card__price-wrapper">';
...
function t_store_addProductQuantity(recid, el_product, product, options) {
    ...
    if ((options.showStoreBtnQuantity === 'list' && el_product.hasClass('t-store__card')) ||
        ...
        if (quantityBtns.length < 1) {
            ...
            str +=          '</div>';
            str +=          '<input class="t-store__prod__quantity-input t-descr t-descr_xxs" type="number" min="1" max="999" step="1" value="1" size="3" maxlength="3" />';
            ...
            /* set default */
            input.val(1);
            input.change(); /* update input, if max is changed */
            var value = input.val();
...
function t_store_initTextAndCharacteristics(el_popup, product) {
    ...
    if (pack_m) {
        prodTxt += '<p class="js-store-prod-weight">' + t_store_dict('product-weight') + ': ' + pack_m + '&nbsp;' + t_store_dict('g') + '</p>';
    }

    if (prodTxt) {
        el_popup.find('.js-store-prod-text').html(prodTxt).show();
    } else {
        el_popup.find('.js-store-prod-text').html('').hide();
    }
...
function t_store_get_control_option_html(options) {
    ...
    str +=
        '    <div class="t-product__option-variants"> <select class="js-product-option-variants t-product__option-select t-descr t-descr_xxs"> [[optiontags]] </select> </div>';
    str += '</div>';
    return str;
}

function t_store_get_control_editionOption_html(options) {
    var str = '';
    ...
    str += '<div class="js-product-edition-option t-product__option" data-edition-option-id="[[id]]">';
    str += '    <div class="js-product-edition-option-name t-product__option-title t-descr t-descr_xxs" ' + styleAttr + '>[[name]]</div>';
    str +=
        '    <div class="t-product__option-variants"> <select class="js-product-edition-option-variants t-product__option-select t-descr t-descr_xxs"> [[optiontags]] </select> </div>';
    str += '</div>';
    return str;
}
...
function t_store_product_initEditions(recid, product, el_product, options) {
    ...
    t_store_product_addEditionControls(product, optionsWrapper, options);
    ...
    el_product.find('.js-product-edition-option').on('change', function () {
        ...
        if (edition) {
            t_store_product_updateEdition(recid, el_product, edition, product);
...
function t_store_product_addEditionControls(product, optionsWrapper, options) {
    if (!product.editionOptions) {
        product.editionOptions = t_store_product_getEditionOptionsArr(product);
    }

    product.editionOptions.forEach(function (curOption) {
        t_store_product_addOneOptionsControl('editionopt', curOption, optionsWrapper, options);
    });
}
...
function t_store_product_updateEdition(recid, el_product, edition, product) {
    /* change price */
    ...
    try {
        ...
        if (edition.pack_x && edition.pack_y && edition.pack_z) {
            ...
            if (defpackobj && defpackobj.pack_x) {
                el_product
        ...
        if (edition.pack_m) {
            ...
            if (defpackobj && defpackobj.pack_m) {
...
function t_store_product_updateEdition_moveSlider(recid, el_product, edition) {
    ...
    /* disable move animation on edition change */
    var moveAnim = sliderWrapper.hasClass('t-slds_animated-fast') ? 'fast' : sliderWrapper.hasClass('t-slds_animated-slow') ? 'slow' : '';
    if (moveAnim) {
        sliderWrapper.removeClass('t-slds_animated-fast, t-slds_animated-slow').addClass('t-slds_animated-none');
        setTimeout(function () {
            sliderWrapper.removeClass('t-slds_animated-none').addClass('t-slds_animated-' + moveAnim);
        }, 800);
    } else {
        setTimeout(function () {
            sliderWrapper.removeClass('t-slds_animated-none');
        }, 500);
    }
    /* move slider */
...
function t_store_product_addOneOptionsControl(type, curOption, optionsWrapper, options) {
    if (curOption.name) {
        var str;
        var tplOneOptionTag;
        var tplSelect;
        ...
        if (type === 'modificator') {
            ...
            tplOneOptionTag = '<option value="[[value]]" data-product-variant-price="[[price]]">[[value]]</option>';
            ...
            $.each(valuesArr, function (key, value) {
                ...
                optionsTags += tplOneOptionTag
                    .replace(/\[\[value\]\]/g, t_store_escapeQuote(valueText))
            ...
            /* edition option */
            /* generate options tags for select */
            tplOneOptionTag = '<option value="[[value]]" >[[value]]</option>';
            $.each(curOption.values, function (key, value) {
                if (value !== '') {
                    optionsTags += tplOneOptionTag.replace(/\[\[value\]\]/g, t_store_escapeQuote(value));
                }
            });
            if (optionsTags!=='') {
                tplSelect = t_store_get_control_editionOption_html(options);
                str = tplSelect
                    .replace(/\[\[id\]\]/g, curOption.id)
...
function t_store_product_getEditionOptionsArr(product) {
...
function t_store_filters_send(recid, opts) {
    ...
    if (sortV === '') {
        ...
        if (t_store_isQueryInAddressBar('s_sort')) {
...
function t_store_filters_opts_getHtml_customSelect(f, opts) {
    ...
    str += '<div class="' + sortClass + ' t-store__filter__item t-store__filter__item_select js-store-filter-item t-descr t-descr_xxs">';
    ...
    str += '        <div class="t-store__filter__item-controls-container" data-type="selectbox">';
    ...
    if (f.values) {
        ...
        for (var j = 0; j < f.values.length; j++) {
            ...
            var className = opts.sidebar && (j > 9) ? 't-store__filter__custom-sel_hidden ' : '';
...
function t_store_filters_opts_getHtml_checkbox(f, opts) {
    ...
    if (f.name === 'quantity') {
        ...
        // str += '<div class="t-store__filter__item-title js-store-filter-item-title" data-filter-name='+ (f.label ? f.label.toLowerCase() : t_store_dict('filter-available-name').toLowerCase()) +'>';
        ...
        /* control = checkboxes */
        str += '<div class="t-store__filter__item t-store__filter__item_checkbox js-store-filter-item t-descr t-descr_xxs">';
        str +=
            ...
            '</div>';
        str += '    <div class="t-store__filter__item-controls-wrap js-store-filter-item-controls-wr">';
        str += '        <div class="t-store__filter__item-controls-container" data-type="checkbox">';
        ...
        if (f.values) {
            ...
            for (var j = 0; j < f.values.length; j++) {
                var v = f.values[j].value;
                var className = opts.sidebar && (j > 9) ? 't-checkbox__control_hidden ' : '';

                if (v !== '') {
                    var tmp = '';
                    tmp = '<label class="t-checkbox__control ' + className + 't-descr t-descr_xxs">'
                        + '<input class="t-checkbox js-store-filter-opt-chb" type="checkbox" name="" data-filter-value="">'
...
function t_store_filters_opts_getHtml_sliderRange(f) {
    ...
    str +=
        '<input class="t-store__filter__range t-store__filter__range_min" type="range" name="price_range" min="' + f.min + '" max="' + f.max + '" data-min-val="' +
        f.min +
        '" value="' +
        t_prod__cleanPrice(f.min) +
        '">';
    str +=
        '<input class="t-store__filter__range t-store__filter__range_max" type="range" name="price_range" min="' + f.min + '" max="' + f.max + '" data-max-val="' +
        f.max +
        '" value="' +
...
function t_store_filters_opts_chosenVal_add(recid, val, el_control, label) {
    var tmp = '';
    tmp = '<div class="t-store__filter__chosen-val js-store-filter-chosen-item t-descr t-descr_xxs" data-field-val="">'
    + '<span class="t-store__filter__chosen-val-close">✕</span>&nbsp;'
    + (label ? label : val)
    + '</div>';
    ...
    var el_chosenTagItem = $('.js-store-filter-chosen-item[data-field-val="' + val + '"]');
...
function t_store_filters_opts_chosenVal_hide(el_rec, value) {
    ...
    el_rec.find('.js-store-filter-chosen-item[data-field-val="' + value + '"]').remove();
    if (el_rec.find('.js-store-filter-chosen-item').length <= 1) {
        el_rec.find('.js-store-filter-reset').removeClass('t-store__filter__reset_visible');
    }
}

function t_store_filters_opts_checkedValues_hideOnClick(recid) {
    var el_rec = $('#rec' + recid);
    var el_filterWrapper = el_rec.find('.js-store-parts-select-container');
    var defVal;
...
function t_store_oneProduct_init(recid, opts) {
    ...
    if (!window.tStoreSingleProductsIsRequested || requestOnBlockChangeInRedactor) {
        ...
        if (window.tStoreSingleProdsObj) {
            t_store_oneProduct_fill(recid, window.tStoreSingleProdsObj[uid], opts);
        } else {
            el_prod.bind('tStoreSingleProductsLoaded', function () {
                t_store_oneProduct_fill(recid, window.tStoreSingleProdsObj[uid], opts);
...
function t_store_updateUrlWithParams(method, key, value, recid) {
    try {
        ...
        var paramsString = window.location.search;
        var searchParams = new URLSearchParams(paramsString);
        var recidInURL = searchParams.get('s_recid');

        var currentParamValue;
        if (typeof value === 'string') {
            // change plus symbol to specsimbol
            value = value.replace('+', '::');
        }
        switch (method) {
            case 'add':
                if (!recidInURL) {
                    searchParams.append('s_recid', recid);
                } else if (recidInURL !== recid) {
                    searchParams.set('s_recid', recid);
                }
                if (searchParams.has(key)) {
                    currentParamValue = searchParams.get(key);
                    searchParams.set(key, currentParamValue + '+' + value);
                } else {
            ...
            case 'delete':
                ...
                if (!currentParamValue) {
                    console.log("can't delete this params from URL");
                    return;
                }
                var valueIndex = currentParamValue.indexOf(value);

                if (valueIndex !== -1) {
                    var currentParamValueArray = currentParamValue.split('+');
                    var removeIndex = currentParamValueArray.indexOf(value);
                    currentParamValueArray.splice(removeIndex, 1);
                    newValue = currentParamValueArray.join('+');
...
function t_store_updateOptionsBasedOnUrl(opts) {
    try {
        var paramsString = window.location.search;
        var searchParams = new URLSearchParams(paramsString);
        ...
        searchParams.forEach(function (value, key) {
            key = t_store_removePrefixFromParamKey(key);
            if (key === 'recid') {
                return;
            }

            if (key.indexOf('price:m') === -1 && key.indexOf('query') === -1 && key.indexOf('quantity') === -1) {
                value = value.split('+');
            }
            ...
            if (key !== 'sort') {
                ...
                opts.sort = {};
                value = value.join();
                var filterValueArray = value.split(':');
                var sortKey = filterValueArray[0];
                var sortValue = filterValueArray[1];
                opts.sort[sortKey] = sortValue;
            }
        });
        return opts;
    } catch (e) {
            ...
            var sorted = el_controls.sort(function(a,b) {
                ...
                if (first.is(':checked') && !second.is(':checked'))
                    ...
                    return 1;
                     ...
                     if (first.attr('data-filter-value') < second.attr('data-filter-value')) {
                         return -1;
                     }
                     else if (first.attr('data-filter-value') > second.attr('data-filter-value')) {
                        return 1;
                    };
                return 0;
            });
            ...
            filterObject[filterKey].forEach(function (filterValue) {
                ...
                if (filterKey === 'price:min') {
                    ...
                    switch (controlType) {
                        case 'checkbox':
                            el_control.prop('checked', true);
                            t_store_filters_opts_chosenVal_add(recid, filterValue, el_control);
                            t_store_filters_opts_checkboxes_changeHiddenInput(el_control);
                            break;
                        case 'selectbox':
                            el_control.addClass('active');
                            t_store_filters_opts_chosenVal_add(recid, filterValue, el_control);
