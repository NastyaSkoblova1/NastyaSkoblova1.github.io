function t_zeroGallery__init(recid, elemid) {
	...
	t_zeroGallery__onSldsLoad('t_sldsInit', function() {
		t_sldsInit(el);
	});
	...
	$('#rec' + recid).find('.t396').unbind('displayChanged', t_zeroGallery__updateSliderHandler);
	$('#rec' + recid).find('.t396').bind('displayChanged', t_zeroGallery__updateSliderHandler);

	el.find('.t-slds').css('visibility', 'visible');
}

function t_zeroGallery__updateSliderHandler() {
	t_slds_updateSlider($(this));
}
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (imgData.li_vimeo || imgData.li_youtube) {
			...
			if (imgData.li_youtube) {
				...
				videoFrame = '<iframe class="tn-elem__gallery__iframe" width="100%" height="100%" src="https://www.youtube.com/embed/' + videoId + '" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
			} else {
				var videoId = imgData.li_vimeo;
				if (videoId == false) videoId = '';
				videoFrame = '<iframe class="tn-elem__gallery__iframe" width="100%" height="100%" src=\"https://player.vimeo.com/video/' + videoId + '" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>';
			}
			...
			if ((imgData.li_img !== undefined && imgData.li_img !== '') || imgData.li_imgurl !== '' || imgData.li_imgtitle !== '') {
				el.find('.t-slds__items-wrapper').append(' \
					<div class="t-slds__item' + (i === 0 ? ' t-slds__item_active' : '') + '" data-slide-index="' + (i + 1) + '"> \
						<div class="t-width"> \
							<div itemscope class="t-null__slds-wrapper t-slds__wrapper t-slds__wrapper_100 t-align_center"> \
								<meta itemprop="image" content="' + escapeHtml(imgData.li_imgalt) + '"> \
								<meta itemprop="caption" content="SEO"> \
								<' + (imgData.li_imgurl ? ('a href="' + encodeURI(imgData.li_imgurl) + '"') : 'div') + ' data-img-lid="' + imgData.lid + '" \
...
function t_zeroGallery__renderViewOneField(el, recid, field) {
	...
	if (field == 'slds_stretch') {
		...
		if (value == '') {
			...
			el.find('.tn-atom').css('border-width', parseInt(value, 10) + 'px');
		}
	}
	...
	if (field == 'slds_arrowcontrols') {
		if (value == '') value = 'near';
		el.removeClass('tn-elem__gallery_arrows-in tn-elem__gallery_arrows-near tn-elem__gallery_arrows-none tn-elem__gallery_arrows-above tn-elem__gallery_arrows-under');
		...
		if (value == 'above' || value == 'under') {
			...
			el.find('.t-slds__arrow_container').css('width', arrowsize * 2 + parseInt(arrowoffset, 10));
		} else {
	...
	if (field == 'slds_dotshmargin') {
		...
		el.find('.t-slds__container').append('\
			<style class="tn-elem__gallery__bullet-hmargin-styles">\
				#rec' + recid + ' .tn-elem[data-elem-id="' + el.attr('data-elem-id') + '"] .t-slds__bullet {\
					margin: 10px ' + value / 2 + 'px;\
				}\
			</style>\
		');
	}
	...
	if (field == 'slds_dotscontrols') {
		...
		var dotsSize = parseInt(t396_elem__getFieldValue(el, 'slds_dotssize'), 10);
		if (value == 'near') {
		...
		el.find('.t-slds__container').append('\
			<style class="tn-elem__gallery__icon-size-styles">\
				#rec' + recid + ' .tn-elem[data-elem-id="' + el.attr('data-elem-id') + '"] .tn-elem__gallery__play {\
					width: ' + parseInt(value, 10) + 'px;\
					height: ' + parseInt(value, 10) + 'px;\
					left: calc(50% - ' + parseInt(value, 10) / 2 + 'px);\
					top: calc(50% - ' + parseInt(value, 10) / 2 + 'px);\
				}\
			</style>\
		');
	}
	...
	if (field == 'slds_arrowsize') {
		...
		el.find('.t-slds__arrow_body').attr('style', 'width: ' + parseInt(value, 10) + 'px !important;');
	...
	if (field == 'slds_arrowlinesize') {
		if (value == '') value = '1';
		el.find('.t-slds__arrow_body polyline, .t-slds__arrow_body path').css('stroke-width', parseInt(value, 10) + 'px');
	...
	if (field == 'slds_dotsbgcolor') {
		if (value == '') value = '#fff';
		el.find('.t-slds__bullet_body').css('background-color', value);
	}
	if (field == 'slds_dotsbgcoloractive') {
		...
		el.find('.t-slds__bullet_wrapper').append('\
			<style>\
				#rec' + recid + ' .tn-elem[data-elem-id="' + el.attr('data-elem-id') + '"] .t-slds__bullet_active .t-slds__bullet_body{\
	...
	if (field == 'slds_arrowalign') {
		...
		el.find('.t-slds__arrow_container').removeClass('t-slds__arrow_container-left t-slds__arrow_container-center t-slds__arrow_container-right');
		...
		el.find('.t-slds__arrow').hover(
			function () {
				...
				var color = value != 'transparent' ?
					('rgba(' + t396_hex2rgba(value, arrowbgopacity) + ')') :
					value;
			...
			function () {
				...
				$(this).css('background-color', colorpre);
			}
		);
	}
	...
	if (field == 'slds_captiontopmargin') {
		...
		if (dotsControls === 'near') {
			...
			topOffset = 20 + parseInt(dotsSize, 10) + parseInt(dotsMargin, 10);
		}
		if (arrowControls === 'under') {
			...
			if (arrowsize == '' || typeof arrowsize == 'undefined') arrowsize = 's';
			arrowsize = t_zeroGallery__getArrowSize(arrowsize);
			topOffset = topOffset + parseInt(arrowsize, 10) + parseInt(arrowoffset, 10);
		}
		...
		el.find('.t-slds__container').append('\
			<style class="tn-elem__gallery__caption-position-styles">\
				#rec' + recid + ' .tn-elem[data-elem-id="' + el.attr('data-elem-id') + '"] .t-slds__caption {\
					margin-top: ' + (topOffset + parseInt(value, 10)) + 'px\
				}\
			</style>\
		');
	}
	...
	if (field == 'slds_arrowvmargin') {
		...
		if (arrowControls === 'above') {
			...
			if (dotsControls === 'near') {
				var dotsMargin = t396_elem__getFieldValue(el, 'slds_dotsvmargin');
				var dotsSize = t396_elem__getFieldValue(el, 'slds_dotssize');
				dotsWrapperHeight = 20 + parseInt(dotsSize, 10) + parseInt(dotsMargin, 10);
			}
			value = parseInt(value, 10) + dotsWrapperHeight;
...
function t_zeroGallery__onSldsLoad(funcName, okFunc, time) {
